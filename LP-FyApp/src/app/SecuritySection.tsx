...
export default function SecuritySection() {
  return (
    <section id="security" className="py-32 px-8 bg-gradient-to-br from-[#FAFAF8] to-[#F5F1E8]">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        {/* Text Content */}
        <div className="space-y-8">
          <h3 className="text-4xl font-bold text-[#2C2C2C]">Segurança em Primeiro Lugar</h3>

          <p className="text-lg text-[#8B8B8B] leading-relaxed">
            Sua segurança financeira é nossa prioridade. FyApp utiliza as tecnologias mais avançadas de proteção de dados.
          </p>

          <ul className="space-y-3">
            {securityFeatures.map((feature, index) => (
              <li key={index} className="flex items-start gap-3 text-lg text-[#8B8B8B]">
                <Lock className="w-6 h-6 text-[#7E8C54] flex-shrink-0 mt-1" />
                <span>{feature}</span>
              </li>
            ))}
          </ul>
