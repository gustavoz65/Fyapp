        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {steps.map((step) => (
            <div key={step.number} className="flex flex-col text-center">
              <div className="w-20 h-20 bg-[#7E8C54] text-white rounded-full flex items-center justify-center text-4xl font-bold mx-auto mb-6">
                {step.number}
              </div>
              <h3 className="text-2xl font-bold text-[#2C2C2C] mb-3 font-playfair">
                {step.title}
              </h3>
              <p className="text-[#8B8B8B] leading-relaxed mb-6 flex-1">
                {step.description}
              </p>
              <img
                src={step.image}
                alt={`Passo ${step.number}`}
                className="w-full rounded-xl shadow-lg mt-auto"
              />
            </div>
          ))}
        </div>
