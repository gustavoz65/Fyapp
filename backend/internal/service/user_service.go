...
func (s *UserService) UpdateSettings(ctx context.Context, userID uuid.UUID, req *model.UpdateUserSettingsRequest) (*model.UserSettings, error) {
	...
	if req.BillReminderDays != nil {
		settings.BillReminderDays = *req.BillReminderDays
	}
	if req.WeeklySummary != nil {
		settings.WeeklySummary = *req.WeeklySummary
	}
	if req.MonthlyReport != nil {
		settings.MonthlyReport = *req.MonthlyReport
	}
	if req.LowBalanceAlert != nil {
		settings.LowBalanceAlert = *req.LowBalanceAlert
	}
