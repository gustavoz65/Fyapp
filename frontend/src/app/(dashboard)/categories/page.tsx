...
export default function CategoriesPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", type: "expense" as string, color: "#3b82f6", icon: "tag" });
  ...
  function openEdit(cat: Category) {
    setEditing(cat);
    setForm({ name: cat.name, description: cat.description || "", type: cat.type, color: cat.color, icon: cat.icon });
    setDialogOpen(true);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        const body: UpdateCategoryRequest = { name: form.name, description: form.description || undefined, color: form.color, icon: form.icon };
        await api.put(`/categories/${editing.id}`, body);
        toast.success("Categoria atualizada");
      } else {
        const body: CreateCategoryRequest = { name: form.name, description: form.description || undefined, type: form.type as CreateCategoryRequest["type"], color: form.color, icon: form.icon };
                  ...
                  <CardContent className="flex items-center justify-between p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {cat.is_system && <Badge variant="outline" className="text-xs">Sistema</Badge>}
                      </div>
                    </div>
                    {!cat.is_system && (
                      <div className="flex gap-1">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader><AlertDialogTitle>Remover categoria?</AlertDialogTitle><AlertDialogDescription>Essa acao nao pode ser desfeita.</AlertDialogDescription></AlertDialogHeader>
                            <AlertDialogFooter><AlertDialogCancel>Cancelar</AlertDialogCancel><AlertDialogAction onClick={() => handleDelete(cat.id)}>Remover</AlertDialogAction></AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                      </div>
                    )}
